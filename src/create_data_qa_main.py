...
for idx in range(len(answers)):
    print(idx)
    new_row = {'context': rules[idx], 'question': questions[idx], 'answer': answers[idx]}
    df = df.append(new_row, ignore_index=True)
    
df = df.reset_index(drop=True)
print(df)

#df.to_csv('B:\\TT\\Project - information extraction\\data\\data1006_QA\\rule_qa.csv')
df.to_csv('B:\\TT\\Project - information extraction\\data\\data1006_QA\\rule_qa_val.csv')
